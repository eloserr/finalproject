...
class Main(tk.Frame):
    ...
    def records(self, name, phone, email, salary):
        self.db.insert_date(name, phone, email, salary)
        self.view_records()
    ...
    def update_record(self, name, phone, email, salary):
        ...
        self.db.cur.execute(
            """UPDATE Staff SET name=?,  phone=?,  email=?, salary=? WHERE id=?""",
    ...
    def delete_records(self):
        for row in self.tree.selection():
            self.db.cur.execute(
                """DELETE FROM Staff WHERE id=?""", (self.tree.set(row, "#1"),)
            )
            self.db.conn.commit()
            self.view_records()
    ...
    def search_records(self, name):
        name = "%" + name + "%"
        self.db.cur.execute("""SELECT * FROM Staff WHERE name LIKE ?""", (name,))
...
class DB:
    ...
    def insert_date(self, name, phone, email, salary):
        self.cur.execute(
